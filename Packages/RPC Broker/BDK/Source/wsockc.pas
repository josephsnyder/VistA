...
begin
  ...
  BufRecv := PAnsiChar(StrAlloc(Buffer32k));
  try // BufRecv
      BufSend := StrNew(PAnsiChar(AnsiString(imsg))); //p60
    try // BufSend
      Result := PChar('');
      ...
      begin
        ...
        BadXfer := False;
        SocketError := send(hSocket, BufSend^, StrLen(BufSend),0); //p60
      ...
      StrDispose(BufSend); //p60
    end; //try BufSend
    if BadXfer then
    begin
      NetError(StrPas('Repeated Incomplete Reads on the server'), XWB_BadReads);  //p60
      Result := StrNew('');
    end; //if BadXfer
