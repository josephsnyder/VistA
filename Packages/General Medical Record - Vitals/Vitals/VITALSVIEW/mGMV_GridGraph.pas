 ...
 * 
 * *****************  Version 2  *****************
 * User: Vhaishandria Date: 8/20/09    Time: 10:15a
 ...
 * 
 * *****************  Version 1  *****************
 * User: Vhaishandria Date: 3/09/09    Time: 3:39p
 ...
 * 
 * *****************  Version 3  *****************
 * User: Vhaishandria Date: 1/07/08    Time: 6:52p
 * Updated in $/Vitals GUI 2007/Vitals/VITALSVIEW
 *
 * *****************  Version 2  *****************
 ...
 *
 * *****************  Version 1  *****************
 * User: Vhaishandria Date: 5/14/07    Time: 10:31a
 * Created in $/Vitals GUI 2007/Vitals-5-0-18/VITALSVIEW
 * 
 * *****************  Version 2  *****************
 * User: Vhaishandria Date: 6/13/06    Time: 11:15a
 * Updated in $/Vitals/VITALS-5-0-18/VitalsView
 * 
 * *****************  Version 1  *****************
 * User: Vhaishandria Date: 5/16/06    Time: 5:44p
        ...
        for i := VMEntry.Count - 1 downto 0 do
          begin
            ...
            if __s = '/' then
              ...
              Cells[c, rPOx+1{5}] := __s+ ' '+s2; // vhaishandria 060727
...
var
  ...
  begin
    try
      ...
      ss := ReplaceStr(ss,'-','/');
      // vhaishandria 061228 - replacement for chart only. to fix position
...
begin
  ...
  setTrackBarLimits;
  // the next line was uncommented to fix Remedy 281003. vhaishandria 2008-11-03
  CurrentPoint := getDefaultGridPosition; //commented  vhaishandria 2008-08-07
...
begin
  Result := (GetWindowlong(grdVitals.Handle, GWL_STYLE) and WS_VSCROLL) <> 0;// vhaishandria 20081031
end;
...
//  if iIgnoreCount > 0 then Exit; //vhaishandria 2008-08-07
  Inc(iIgnoreCount);
...
begin
  ...
  aComments := 'No Reload';
  try
    if Reload then  // vhaishandria 060609
      begin
        aComments := 'Reload';
...
begin
  if iIgnoreCount > 0 then Exit;
  iPos := grdVitals.LeftCol; // vhaishandria 2008-04-14
  ...
  grdVitals.LeftCol := iPos; // vhaishandria 2008-04-14

  // vhaishandria 2008-08-07 last minute fix for Patch 5.0.22.7
  // axis X min is not set correctly when in Sychro mode
  CurrentPoint := trbHGraph.Position;// vhaishandria 2008-08-07
end;
  ...
  begin
    ...
    for i := 0 to  AllVitalsData.Count - 1 do
      begin
        ...
        sDate := Piece(s,'^',iDate);
        sTime := Piece(s,'^',iTime);
// 061228 vhaishandria uncommening 2 lines
        if i = 0 then
...
//    VitalsData.Add('  Date:' + #9 + grdVitals.Cells[iCol, 0]);
    ...
    if cbChrono.Checked then d := Series.XValue[ValueIndex]
      ...
      begin
        ...
        // 061228 vhaishandria ......
        // vhaishandria 061228 - replacement was done to fix position on the chart
...
//              Canvas.Brush.Color :=  fBGColor; //DISPLAYCOLORS[GMVNormalBkgd];
      ...
      else if (aRow = 5) then // vhaishandria 2008-01-07 ----------------- begin
      // qualifiers are shown regardless of user preferences(options)
        begin
          if pos('*',trim(Cells[ACol, 4])) >0 then
            ...
            Value := '';
          Canvas.TextRect(Rect, Rect.Left+10, Rect.Top, Value);
        end // vhaishandria 2008-01-07 ------------------------------------- end
      else if AbnormalValue then
        begin
...
//    if ss = '' then               //  vhaishandria 20090814
...
var
  aRow: Integer;
begin
  if aABBR = 'BMI' then aRow := rBMI{8} // vhaishandria 060913 BMI selection
  else
  ...
  //    vtBMI: aRow := 8; // vhaishandria 060913 BMI selection
...
begin
  ...
  if not bData then
    ...
    //  Page number
      ...
      SL.Insert(0,'');
      SL.Insert(0,'Location: '+ lblHospital.Caption); // vhaishandria 061116
      SL.Insert(0,ptInfo); // vhaishandria 061116
...
begin
  ...
  for i := iStart to iEnd do
    begin
      ...
      if Trim(sLine) <> '' then  // R141401 - vhaishandria 060921 --------------
