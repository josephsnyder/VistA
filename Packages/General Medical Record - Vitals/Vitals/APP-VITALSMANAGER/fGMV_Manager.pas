...
uses
  ...
  , uGMV_Template, AppEvnts //uROR_Contextor, 
  ;
...
uses
  ...
  , fROR_PCall
  , uGMV_Engine, uROR_RPCBroker, uGMV_RPC_Names, fGMV_RPCLog;
...
{$ENDIF}
  ...
  if Result then // vhaishandia 2007-07-18
...
//  with tv.Items.AddChildObject(FAbnormalRoot, 'O2 Sat', TGMV_VitalHiLoDefinition.Create(hltO2Sat, 0, 100, 10)) do
  ...
  try
    tmpList := getTemplateList; // vhaishandria 05-01-13
...
//        tv.SetFocus; vhaishandria 20090304
      end;
  except
    on E: Exception do
...
function TfrmGMV_Manager.ApplicationEventsHelp(Command: Word;
