 ...
 * 
 * *****************  Version 2  *****************
 * User: Vhaishandria Date: 7/18/07    Time: 12:42p
...
type
  ...
  private
    ...
    FDDNumber: string;
    FFileName: string;
    fLoadedLast: string; // vhaishandria 060801
    fLoaded: Boolean; // vhaishandria 060801
  ...
  public
    ...
    function LoadEntriesStartCount(FileNumber, StartNumber: string;
      Count: Integer): Integer;   // vhaishandria 060801
    procedure Display;
...
begin
  ...
  fLoaded := False; // vhaishandria 060801
  fLoadedLast := ''; // vhaishandria 060801
end;
...
begin
  ...
  repeat
    ...
    if RetList.Count > 1 then
      begin
        FFileName := Copy(RetList[0], Pos('^', RetList[0]), Length(RetList[0]));
        for i := 1 to RetList.Count - 1 do  // vhaishandria 060801
          begin
            fe := TGMV_FileEntry.CreateFromRPC(RetList[i]);
            FTypes.AddObject(fe.CaptionConverted, fe);
            fLoadedLast := fe.FIEN;
