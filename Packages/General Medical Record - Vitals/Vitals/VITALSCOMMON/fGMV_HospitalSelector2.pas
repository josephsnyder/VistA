...
// vhaishandria 2007-10-04
// uncomment the next line to correlate search delay with user preference value
//  GMVSearchDelay := GMVUser.Setting[usSearchDelay];// vhaishandria 2007-10-04
  ...
  try
    ...
    for i := 0 to sl.Count - 1 do
      begin
        ...
        ss := Piece(s,'^',2);
        ii := StrToIntDef(ss,-1);
        if ii < 1 then continue; // vhaishandria 060112
...
begin
  ...
  btnOK.Enabled := readyToSelect;
  if Assigned(lv.Selected) then // vhaishandria 2007-07-16
  try
    fID := lv.Selected.SubItems[0];
    fName := lv.Selected.Caption;
    edSelected.Text := fName;
