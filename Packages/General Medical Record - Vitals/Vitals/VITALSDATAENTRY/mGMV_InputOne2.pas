 ...
 * 
 * *****************  Version 4  *****************
 * User: Vhaishandria Date: 2/01/08    Time: 7:39a
...
//  case StrToIntDef(FTemplateVital.IEN, -1) of
  ...
  case aType of
    ...
    vtPO2:
    // vhaishandria 060613 - Room Air question
      begin
        FTemplateVitalType := vtPO2;
// vhaishandria 060920 Remedy 146205 - Room Air can not be selected as the default
//        lblQualifiers.Caption := '[]'; //vhaishandria 060725 Remedy 150714, 151694, 146205
...
begin
  cbxInput.Text := trim(cbxInput.Text);// Remedy 170801  vhaishandria 061214
      ...
      if GetSupplementO2Data(PO2FlowRate, PO2Percentage, TControl(Sender),lblQualifiers.Caption) then
        begin
          ...
          if FPO2Percentage <> '' then
            s := s + ' ' + FPO2Percentage + ' %';
          if s = ' ' then s := '';
// 20071220 vhaishandria --- all flow rates and %% values are included --- /////
...
begin
  ...
  cbxInput.Hint := HintString(ckbMetric.Checked);// vhaishandria 2003/06/06
...
//      if sValue + sPercent + sFlow = '' then  // vhaishandria 2008-01-24
      if sValue = '' then
        Result := ''
      else
        Result := GMVVitalTypeIEN[FTemplateVitalType] + ';' + cbxInput.Text +
...
begin
    ...
    sValue := aString;
    _Double := 0; // vhaishandria 2007-07-12
    if not aMetric and
