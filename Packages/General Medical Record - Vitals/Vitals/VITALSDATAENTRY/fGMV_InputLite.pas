...
function VitalsInputDLG(aPatient, aLocation, aTemplate,aSignature:ShortString;
...
var
  ...
  begin
    ...
    if Piece(s, '^', 1) = '-1' then
      MessageDlg('Unable to set parameter ' + _WindowName + '.' + #13 +
...
begin
  try
    ...
    frmGMV_InputLite.pnlButtons.Width := frmGMV_InputLite.btnSave.Width +
      frmGMV_InputLite.btnSave.Left + 6;
...
{$IFDEF DLL}
      chkbCloseOnSave.Visible := False; // vhaishandria 050209
      btnSaveAndExit.Visible := true;

//      CurrentTime := Now;
      if aDateTime <> 0 then
        begin
          CurrentTime := aDateTime; //vhaishandra 20081008 fixing discepency between PC and VistA
      ...
      while piece(FTemplateData, ';', i) <> '' do
        begin
          try
            ...
            InputOne2.DFN := fDFN; // vhaishandria 2009 02 27
...
procedure TfrmGMV_InputLite.acSavePreferencesExecute(Sender: TObject);
  ...
  procedure saveBoolean(aSetting: TGMV_UserSetting;aValue:Boolean;
    aTrue,aFalse:String);
  begin
    if aValue then
...
begin
  ...
  if chkOneUnavailableBox.Checked then GMVUser.Setting[usOneUnavailableBox] :='OneUnavailableBox'
  else                  GMVUser.Setting[usOneUnavailableBox] :='ManyUnavailableBoxes';
(*
...
//  else                  GMVUser.Setting[usTemplateTree] := 'NoTemplates';
  ...
  if  bShowVitals then  GMVUser.Setting[usLastVitals] := 'ShowLastVitals'
  else                  GMVUser.Setting[usLastVitals] := 'NoLatestVitals';
...
(*  commented 2008-04-23 vhaishandria
...
{ vhaishandria 20050209
  ...
  cbU.Checked := s <> 'OFF';
{vhaishandria 2008-04-23 commented
...
{$ENDIF}
  for i := 1 to TheSL.Count-1 do // first row contains headers
    begin
      s := TheSL.Strings[i];
      for j := 0 to strgLV.ColCount-1 do  strgLV.Cells[j,i] := Piece(s,'^',j+1);
    end;
  TheSL.Free;
        ...
        with TfraGMV_InputOne2(sbxMain.Controls[i]) do
          begin
            ...
            if ckbUnavailable.Checked then
              ...
              begin
                s1 := VitalsRate;
(* vhaishandria 20071220 ==== No special treatment for sPO2 ====================
...
{$IFDEF USEVSMONITOR}
// CASMED support////////////////////////////////////////////////
  // vhaishabdria 20080929 check added to speed up cases when the device is not used
  if bCasmedUsed then
    try
...
var
  ...
  begin
    if lvSelPatients.ItemFocused.SubItems[3] = '' then
      ...
      begin
        lvSelPatients.ItemFocused.SubItems[3] :=
          lvSelPatients.ItemFocused.SubItems[3] + ' ' +IntToStr(anIndex);
  ...
  begin
    ...
    if not Result then
      begin
        ...
        TfraGMV_InputOne2(sbxMain.Components[i]).cbxInput.SelLength :=
          Length(TfraGMV_InputOne2(sbxMain.Components[i]).cbxInput.Text);
        ...
        if bSlave then // DLL
          begin
            if chkbCloseOnSave.Checked
              or (Sender = btnSaveAndExit) // added vhaishandria 050707 for DLL use
              then  Close
            else
...
{$IFDEF USEVSMONITOR}
  ...
  if (fMonitor <> nil) and fMonitor.Active then
    ...
    begin
      FreeAndNil(fMonitor);
      sCasmedPort := '';
      for i := 1 to ParamCount do // vhaishandria 20090112
        begin
...
//         lvSelPatients.Items[i].ImageIndex := indSingleInput;
  ...
  try
    ...
    if lvSelPatients.ItemFocused.SubItems[3] = '' then
      ...
      if pos(' ',lvSelPatients.ItemFocused.SubItems[3]) <> 0 then
  ...
  setPatientInfoView(lvSelPatients.ItemFocused.SubItems[0],
    lvSelPatients.ItemFocused.SubItems[2]); // vhaishandria 060309
...
begin
  ...
  if sKey = 'MODEL' then i := 1;
  if sKey = 'PORT' then i := 2;
  if sKey = 'STATUS' then i := 3;
...
//        Casmed.ClosePort;
    ...
    // vhaishandria 20080929 tracking use of the monitor
    bCasmedUsed := (sBP<>'') or (sPulse<>'') or (sSpO2<>'') or (rTemp<>0) or
      (sTemp<>'') or (sTempUnit<>'');
...
begin
  ShowPatientInfo(FDFN,'Patient Inquiry for: '+edPatientName.Text +
    '  ' + edPatientInfo.Text); // vhaishandria 060308
end;
...
begin
  ShowPatientLatestVitals(FDFN,'Latest Vitals for: '+edPatientName.Text +
    '  ' + edPatientInfo.Text); 
end;
