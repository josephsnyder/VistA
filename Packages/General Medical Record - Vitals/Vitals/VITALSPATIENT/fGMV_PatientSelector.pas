...
type
  ...
  public
//    lbClinics: TListBox;
    lbClinics: TNoVScrollListBox; // vhaishandria 2008-04-17
    { Public declarations }
    bIgnore: Boolean;
    bIgnorePtChange:Boolean; // vhaishandria 2008-03-07
    ...
    property PatientLocationID :String read fPatientLocationID;// vhaishandria 2007-10-04
    ...
    procedure setPatientLocationIDByFirstSelectedPatient; // vhaishandria 2008-02-28
  end;
...
procedure TfrmGMV_PatientSelector.FormCreate(Sender: TObject);
  ...
  // vhaishandria 5.0.23.6 20090218 --------------------------------------------
...
//  lbClinics.Sorted := True;
  ...
  sLastClinicFound := '';
{
  setLV(lvUnitPatients);        // vhaishandria 5.0.23.6 20090218
  setLV(lvWardPatients);        // vhaishandria 5.0.23.6 20090218
  setLV(lvAllPatients);         // vhaishandria 5.0.23.6 20090218
  setLV(lvTeampatients);        // vhaishandria 5.0.23.6 20090218
  setLV(lvClinicPatients);      // vhaishandria 5.0.23.6 20090218
}
end;
...
begin
  bIgnore := True; // vhaishandria 2008-03-07
...
//  tmrLoad.Enabled := true; -- no background load
  ...
  if (lbClinics.Items.Count > 0) and (pcMain.ActivePage= tsClinic) then
    begin
      lbClinics.setFocus;   // vhaishandria 060920 -- commented to avoid RTE
      UpdateScrollBar;
    end;
  bIgnorePtChange := False;
  bIgnore := False; // vhaishandria 2008-03-07
...
begin
  if aCount = 0 then
    ...
    begin
      try
        if GetParentForm(Self).Visible then // vhaishandria 2007-07-16
        GetParentForm(Self).Perform(WM_NEXTDLGCTL, 0, 0);
      except                                // vhaishandria 2007-07-16
      end;                                  // vhaishandria 2007-07-16
...
// vhaishandria 2008-03-10 commenting out message to parent form
...
var
  tmpList: TStringList;
begin
  pcMain.Enabled := False; // vhaishandria 2008-04-15
  ...
  if assigned(fGetList) then
    begin
      ...
      tmpList.Free;
      pcMain.Enabled := True; // vhaishandria 2008-05-12
      if fFound > 0 then
        begin
          fLV.TabStop := true;
          fLV.Enabled := true;
          ...
          try
            ...
            if self.Visible then // vhaishandria 2007-07-16
              fLV.SetFocus;
          except
          end;
        end
  ...
  pcMain.Enabled := True; // vhaishandria 2008-04-15
  if not self.Visible then
    exit; // vhaishandria 2008-11-03
...
begin
  ...
  for i := 0 to fLV.Items.Count - 1 do
    if (fLV.Items[i].Selected)
      and Assigned(fLV.Items[i].Data) // vhaishandria 2007-07-17
    then
...
begin
  iFound := 0;
  lvItems.BeginUpdate;
  b := bIgnorePtChange; // vhaishandria 2008-03-07
  bIgnorePtChange := True; // vhaishandria 2008-03-07
  ...
  lvItems.EndUpdate;
  Result := iFound;
  bIgnorePtChange := b; // vhaishandria 2008-03-07
end;
...
//                fSelectedPatientLocationName := Patient.LocationName; // vhaishandria 060608
//                fSelectedPatientLocationID := Patient.LocationID; // vhaishandria 060608
              end
            else
...
begin
  if (fLV.Selected <> nil)
    and Assigned(fLV.Selected.Data) // vhaishandria 2007-07-17
  then
    begin
      try
...
var
  f: TCustomForm;
begin
//  if bIgnorePtChange then Exit; // vhaishandria 2008-03-07
  SetCurrentPatient;
  SelectionStatus := ssInProgress;
//  setCurrentList; -- commented 2008-02-27 vhaishandria
// vhaishandria 2008-02-27 ----------------------------------------------- begin
  ...
  // vhaishandria 2008-02-27 ------------------------------------------------- end
  setInfo;
//  if not bIgnore then // vhaishandria 2008-03-07
...
begin
  ...
  try
    if (fLV.ItemIndex> - 1) and Assigned(fLV.Items[fLV.ItemIndex].Data) then // vhaishandria 2007-12-16
    Result :=  TGMV_FileEntry(fLV.Items[fLV.ItemIndex].Data).IEN;
  except
  end;
end;

procedure TfrmGMV_PatientSelector.setPatientLocationIDByFirstSelectedPatient; // vhaishandria 2008-02-28
var
  s: String;
begin
            ...
            begin
              if aDirection = '1' then
                begin
                  ...
                  fListBox.Items.Add(SL[iSL]);    // vhaishandria 2008-04-15
                end
              else
                begin
                  GMVClinics.Entries.InsertObject(0,SL[iSL],fe);
                  fListBox.Items.Insert(0,SL[iSL]); // vhaishandria 2008-04-15
                end;
            end;
        end;
      SL.Free;
    ...
    begin
      ...
      if iCount > 0 then
        begin
          ...
          // added vhaishandria 20080701 11:45
          iPos := LookupClinicInTheList(aTarget);
          if iPos >= 0 then
            setPos(iPos,iLen);
        end
...
var
  ...
  begin
    ...
    fListBox.Items.Assign(GMVClinics.Entries); // vhaishandria 20080609
    Result := i;
