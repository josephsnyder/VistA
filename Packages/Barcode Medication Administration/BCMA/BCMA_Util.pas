...
uses SysUtils, Windows, Messages, Classes, Controls, StdCtrls, ExtCtrls,
  ComCtrls, Forms, Graphics, Menus, Dialogs, BCMA_Startup, Splash, StrUtils,
  ShellAPI, Grids;
...
//  ErrIncompleteData = 'Incomplete data returned from VistA';
  ...
  SHARE_DIR = '\VISTA\Common Files\';   // for BCMAOrderCom.dll 2 uOrderMgr
...
function FMDateTimeOffsetBy(ADateTime: TFMDateTime; DaysDiff: Integer):
  TFMDateTime;
...
// force use throughout BCMA of same Piece function found in broker unit MFunStr.
{ $IFDEF CAS_DDPE_TEST}
function Piece(const S: string; Delim: char; PieceNum: Integer): string;
{ $ENDIF}
function Pieces(const S: string; Delim: char; FirstNum, LastNum: Integer):
  string;
procedure PiecesToList(x: string; ADelim: Char; AList: TStrings);

// Count number of pieces in string delimited by ADelim
//function PieceCnt(x: String; ADelim: Char): Integer;
function PieceCnt(x: string; ADelim: string): Integer;

function ReverseStr(const x: string): string;
procedure SetPiece(var x: string; Delim: Char; PieceNum: Integer; const
  NewPiece: string);
procedure SortByPiece(AList: TStringList; ADelim: Char; PieceNum: Integer);
procedure StripPieceIfContains(ADelim: Char; StrToFind: string; var AString,
  DestStr: widestring);
function padr(s: string; n: integer): string;
function StripLB(inStr: string): string; // remove line breaks (CR-LF) from string

{ Display functions }
//procedure ClearControl(AControl: TControl);
// copied from Dialogs.CenterWindow which was implementation-only.  rpk 10/25/2012
//procedure CtrWnd(Wnd: HWnd);
...
procedure SetControlsEnable(parentctrl: TWinControl; inbool: Boolean); // 5/24/2012

{ ListBox Grid functions }
procedure ListGridDrawCell(AListBox: TListBox; AHeader: THeaderControl; ARow,
  AColumn: Integer;
  const x: string; WordWrap: Boolean);
procedure ListGridDrawLines(AListBox: TListBox; AHeader: THeaderControl; Index:
  Integer;
  State: TOwnerDrawState);
function ListGridRowHeight(AListBox: TListBox; AHeader: THeaderControl; ARow,
  AColumn: Integer;
  const x: string): Integer;

// listview function
function getLvwRow(lvw: TListView): String;


//function DefMessageDlg(const Msg: string; DlgType: TMsgDlgType; Buttons:
//  TMsgDlgButtons;
//  HelpCtx: Longint; const aCaption: string = ''): Integer;

// DefMessageDlg should display a message dialog above all other windows of this
// application.  It should be able to go behind windows of other applications
// that are in the foreground.
function DefMessageDlg(const Msg: string; DlgType: TMsgDlgType; Buttons: // rpk 5/14/2013
  TMsgDlgButtons; HelpCtx: THelpContext; const aCaption: string = ''
{$IFDEF R1224198}
  ; NoFocus: Boolean = False
{$ENDIF}
  ): Integer;                           // rpk 5/14/2013
...
// StringGrid functions
procedure sgClear(var sg: TStringGrid);
procedure sgFree(colcnt, rowcnt: Integer; var sg: TStringGrid);
procedure sgInit(sg: TStringGrid; fcnt, incnt: Integer);
...
uses
  BCMA_Objects,
//  MFunStr,
  trpcb, VA508AccessibilityRouter;
...
const
  ...
  ($0, $77073096, $EE0E612C, $990951BA, $76DC419, $706AF48F, $E963A535,
    $9E6495A3,
    $EDB8832, $79DCB8A4, $E0D5E91E, $97D2D988, $9B64C2B, $7EB17CBD, $E7B82D07,
    $90BF1D91,
    ...
    $B6662D3D,
    $76DC4190, $1DB7106, $98D220BC, $EFD5102A, $71B18589, $6B6B51F, $9FBFE4A5,
    $E8B8D433,
    $7807C9A2, $F00F934, $9609A88E, $E10E9818, $7F6A0DBB, $86D3D2D, $91646C97,
    $E6635C01,
    ...
    $C0BA6CAD,
    $EDB88320, $9ABFB3B6, $3B6E20C, $74B1D29A, $EAD54739, $9DD277AF, $4DB2615,
    $73DC1683,
    $E3630B12, $94643B84, $D6D6A3E, $7A6A5AA8, $E40ECF0B, $9309FF9D, $A00AE27,
    $7D079EB1,
    ...
    $5BDEAE1D,
    $9B64C2B0, $EC63F226, $756AA39C, $26D930A, $9C0906A9, $EB0E363F, $72076785,
    $5005713,
...
begin
  Result := -1;
  if (Length(AString) > 0) and IsFMDateTime(AString) then
...
begin
  ...
  begin
    ...
    if Length(x) > 0 then
      x := FormatFMDateTime(AFormat, MakeFMDateTime(x));
...
begin
  Result := False;
  for i := 1 to Length(x) do
    if x[i] in ['!'..'~'] then          // ordinal values 33..126
    begin
      Result := True;
      break;
    end;
...
function UpdateCrc32(Value: DWORD; var Buffer: array of Byte; Count: Integer):
  DWORD;
var
  i: integer;
begin
  ...
  with AList do
    ...
    begin
      ...
      for j := 1 to Length(x) do
        case x[j] of
          #9: for k := 1 to (ATabWidth - (Length(y) mod ATabWidth)) do
              y := y + ' ';
          #32..#127: y := y + x[j];
          #128..#159: y := y + '?';
          #160: y := y + ' ';
...
var
  i: Integer;
begin
  while (Length(x) > 0) and not (x[1] in ['0'..'9']) do
    Delete(x, 1, 1);
  for i := 1 to Length(x) do
    if not (x[i] in ['0'..'9']) then
      break;
...
begin
  ...
  Inc(i);
  if (i < Src.Count) and (Src[i][1] <> '~') then
    repeat
      if Src[i][1] = 'd' then
        Result := Copy(Src[i], 2, 255);
      Inc(i);
...
// piece count = number of delimiters found plus 1

function PieceCnt(x: string; ADelim: string): Integer;
var
  i, cnt, offset, len: Integer;
begin
  offset := 1;
  len := Length(x);
  cnt := 0;
  ...
  if len > 0 then begin
    repeat
      i := PosEx(ADelim, x, offset);
      if i >= offset then begin
        Inc(cnt);
      end;
      offset := i + 1;
    until i = 0;
    // add 1 after last delimiter
    inc(cnt);
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          while (x[SpacePos] <> ' ') and (SpacePos > 1) do
            Dec(SpacePos); {**REV**}    {changed 0 to 1}
          if SpacePos = 1 then
            SpacePos := MaxLength; {**REV**} {changed 0 to 1}
...
function StripLB(inStr: string): string;
var
  tstr: string;
  ipos: Integer;
begin
...
procedure SetControlsEnable(parentctrl: TWinControl; inbool: Boolean);
var
  i: Integer;
begin
  for i := 0 to parentctrl.ControlCount - 1 do
    parentctrl.Controls[i].Enabled := inbool;
end;

function InfoBox(const Text, Caption: string; Flags: Word): Integer;
{ wrap the messagebox object in case we want to modify it later }
begin
  Result := Application.MessageBox(PChar(Text), PChar(Caption), Flags or
    MB_TOPMOST);
end;
...
begin
  ...
  begin
    ...
    if AControl is TEdit then
      AHandle := TEdit(AControl).Font.Handle;
    if AControl is TMemo then
      AHandle := TMemo(AControl).Font.Handle;
    if AControl is TRichEdit then
      AHandle := TRichEdit(AControl).Font.Handle;
    if AHandle = 0 then
      Exit;
    AWidth := TextWidthByFont(AHandle, x);
    ARect := Rect(LEFT_MARGIN, 0, LowerOf(ClientWidth, AWidth + LEFT_MARGIN),
      ClientHeight);
...
begin
  if Application.MainForm <> nil then
    Result := Application.MainForm.Font.Size
  else
    Result := 8;
end;
...
begin
  ...
  for i := 0 to AColumn - 1 do
    ARect.Left := ARect.Left + AHeader.Sections[i].Width;
  Inc(ARect.Left, 2);
  ARect.Right := ARect.Left + AHeader.Sections[AColumn].Width - 6;
  if WordWrap then
    Format := (DT_LEFT or DT_NOPREFIX or DT_WORDBREAK)
  else
    Format := (DT_LEFT or DT_NOPREFIX);
...
var
  icol, irow: Integer;
begin
  for irow := 0 to sg.RowCount - 1 do begin
    for icol := 0 to sg.ColCount - 1 do begin
      if sg.Objects[icol, irow] <> nil then begin
        sg.Objects[icol, irow].Free;
        sg.Objects[icol, irow] := nil;
      end;
    end;
    sg.Rows[irow].Clear;
  end;
...
// fcnt = number of fixed rows (header)
// incnt = number of data rows
// 'Fixed row count must be less than row count'

procedure sgInit(sg: TStringGrid; fcnt, incnt: Integer);
begin
  if incnt > 0 then                     // rpk 10/12/2010
    sg.RowCount := fcnt + incnt         // rpk 10/12/2010
  else
    sg.RowCount := fcnt + 1;            // rpk 10/12/2010
...
var
  str: String;
  i, subcnt, idx: Integer;
begin
  Result := '';
  if Assigned(lvw) then begin
    idx := lvw.ItemIndex;
    if idx >= 0 then begin
      str := lvw.Items[idx].Caption;
      subcnt := lvw.Items[idx].SubItems.Count;
      for I := 0 to subcnt - 1 do
        str := str + ', ' + lvw.Items[idx].SubItems[i];
      Result := str;
    end;
  end;
end;                                    // getLvwRow
...
{$IFDEF R1224198}                       // removing focus from the only button of the dialog
type
  TDLGEventHandlers = class             //bjr - INC1224198 - 6/2/2015
    class procedure FormKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  end;

Class procedure TDLGEventHandlers.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);                  //bjr - INC1224198 - 6/8/2015
var
  btn: TButton;
  frm: TForm;
  iCount,
  ...
  function getBtnCount(aForm: TForm): Integer;
  var
    ii: Integer;
  begin
    Result := 0;
    for ii := 0 to frm.ComponentCount - 1 do
      if aForm.Components[ii] is TButton then
        inc(Result);
  end;
...
begin
  ...
  begin
    ...
    begin
      frm := TForm(Sender);
      iCount := getBtnCount(frm);
      if iCount = 1 then
        for i := 0 to frm.ComponentCount - 1 do
          if frm.Components[i] is TButton then
          begin
            btn := TButton(frm.Components[i]);
            if btn.TabStop = False then
            begin
              btn.TabStop := true;
              frm.SetFocusedControl(btn);
            end;
          end;
    end;
  end;
...
{$IFDEF R1224198}                       // removing focus from the only button of the dialog
  ...
  function getRequestedButtonCount: Integer; //bjr - INC1224198 - 6/8/2015
  var
    iBtn: TMsgDlgBtn;
  begin
    result := 0;
    for iBtn := Low(TmsgDlgBtn) to High(TmsgDlgBtn) do
      if iBtn in Buttons then
        inc(Result);
  end;
{$ENDIF}
...
begin
  ...
  Result := mrNone; // rpk 3/1/2016

  newMsg := StringReplace(Msg, '&', '&&', [rfReplaceAll]);

  if newMsg = '' then
    newMsg := 'Error: A blank message was sent to display in this pop-up. ' + #13
      +
...
{$IFDEF R1224198}                       // removing focus from the only button of the dialog
      if NoFocus then
        iCount := getRequestedButtonCount;
{$ENDIF}

      TButton(Dlg.FindComponent('OK')).Caption := '&OK';
      TButton(Dlg.FindComponent('Cancel')).Caption := '&Cancel'; // rpk 8/18/2010
...
{$IFDEF R1224198}                       // removing focus from the only button of the dialog
      ...
      begin
        dlg.KeyPreview := true;
        dlg.OnKeyUp := TDLGEventHandlers.FormKeyUp;
      end;
{$ENDIF}
...
//      zPnl := TPanel.Create(dlg);  // rpk 7/13/2015
...
//            Caption := newmsg;  // rpk 7/13/2015
          ...
          { DEBUG
          bforeground := ForegroundTask;
          bactive := Dlg.Active;
          bfocused := dlg.Focused;
          if useDebugLog then
            writeLogMessageProc('DefMessageDlg: ' + Msg + CRLF +
              'foreground= ' + FalseTrue[bforeground] +
              ' active= ' + FalseTrue[bactive] +
              ' focused= ' + FalseTrue[bfocused] + CRLF, nil); }
          GetScreenReader.Speak(msg);   // rpk 7/10/2015
...
var
  CurrentVer,
    PrevVer,
    RequiredText,                       //version text to be sent to RPC
    CheckType: string;
  //are we checking for a specific patch, or retrieving the current version of a package
  procedure Incompatible;
  var
    msg: string;
  begin
    msg := 'You must be running version ' + CurrentVer;
    if PrevVer <> '' then
      msg := msg + ' or ' + PrevVer;
    msg := msg +
      ' of GUI BCMA to be compatible with the version of BCMA installed on VistA';
    writeLogMessageProc(msg, nil);
    DefMessageDlg(msg, mtError, [mbOK], 0);
    Result := True;
  end;

  function CheckBuild(VerNum: string): Boolean;
  var
    RequiredBuild: string;
  begin
    Result := True;
    RequiredBuild := piece(VerNum, '.', 4);
    if RequiredBuild = '*' then
      result := false
    else
    try
      if StrToInt(piece(AppFileVersion, '.', 4)) >= StrToInt(RequiredBuild) then
        Result := False
    except
      incompatible;
    end;
  ...
  procedure ServerIncompatible;
  begin
    writeLogMessageProc('Patch ' + RequiredPatch +
      ' is required on the server before using this version of BCMA.', nil);
    DefMessageDlg('Patch ' + RequiredPatch +
      ' is required on the server before using this version of BCMA.',
      mtError, [mbOK], 0);
  end;
...
begin
  ...
  with BCMA_Broker do
  begin
    //    FrmSplash.lblStatus.Caption := 'Checking Server for Required Patches...';
    //    FrmSplash.stStatus.Caption := 'Checking Server for Required Patches...';
    //    FrmSplash.stStatus.Invalidate;  // rpk 6/8/2009
    //    FrmSplash.Update;
    FrmSplash.WriteStatus('Checking Server for Required Patches...');
    // rpk 6/9/2009
    if piece(RequiredPatch, '*', 3) = '' then
    begin
      RequiredText := 'PSB';
      CheckType := 'B';
    end
    ...
    begin
      RequiredText := RequiredPatch;
      CheckType := 'P';
    end;

    if CallServer('PSB CHECK SERVER', [CheckType, RequiredText], nil) then
      if CheckType = 'P' then
      begin
      ...
      else if (piece(Results[0], '^', 1) = '-1') or (piece(Results[0], '^', 2)
        <> RequiredVersion) then
      begin
        ServerIncompatible;
        Result := True;
        Exit;
      end;
    //    FrmSplash.lblStatus.Caption := 'Checking Client for Required Version...';
    //    FrmSplash.stStatus.Caption := 'Checking Client for Required Version...';
    //    FrmSplash.stStatus.Invalidate;  // rpk 6/8/2009
    //    FrmSplash.Update;
    FrmSplash.WriteStatus('Checking Client for Required Version...');
    // rpk 6/9/2009

    if CallServer('PSB VERSION CHECK', [''], nil) then
      ...
      begin
        CurrentVer := piece(results[1], '^', 1);
        PrevVer := piece(results[1], '^', 2);
        if pieces(CurrentVer, '.', 1, 3) = pieces(AppFileVersion, '.', 1, 3)
          then
        begin
          if CheckBuild(CurrentVer) then
            Incompatible
        end
        else if pieces(PrevVer, '.', 1, 3) = pieces(AppFileVersion, '.', 1, 3)
          then
        begin
          if CheckBuild(PrevVer) then
            Incompatible
        end
        else
          Incompatible;
...
begin
  ...
  Result := StringReplace(Result, '|', ' ', [rfReplaceAll]);
  Result := StringReplace(Result, '!~', ' ', [rfReplaceAll]);
  {JK 9/2/2008 - replace CRLF token with space}
  ...
  begin
    ...
    Msg := Msg + 'Text cannot start with a question Mark.' + #13;
    Msg := Msg + 'Text cannot contain the "^" or "|" character.' + #13#13;
    Msg := Msg + 'Text cannot contain non-printable characters.' + #13#13;
    Msg := Msg + 'Text cannot contain the token sequence !~.' + #13#13;
...
function padr(s: string; n: integer): string;
{Pads a string to the right with spaces up to length n.  If the string is greater than n, then
truncates and concatenates an elipsis.}
var
  Len: integer;
begin
  Result := s;
...
begin
  ...
  if ShellExecute(Application.ActiveFormHandle, 'OPEN', PChar(AFile),
    PChar(Param), '', 1) > 32 then
    Result := True
  else
