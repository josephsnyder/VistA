...
{ $ENDIF}
  ...
  private
    ...
    FDivisionIEN,
    FDivisionNum, // rpk 4/14/2011
//      FSiteIEN,
      FDivisionName,
      FInstructorDUZ: string;
    //If this is a student, then this contains the DUZ of the instructor
...
{$IFDEF CAS_DDPE_TEST}      // zzzzzzandria 2015-01-29 ===============================
      anEvent := nil;
      try
        aStart := Now;
...
//            FDivisionNum := piece(Results[7], '^', 3);
            // corrected to match PSB USRLOAD; rpk 5/2/2011
            FDivisionNum := piece(Results[7], '^', 2);
//{$IFDEF TEST_ON}
//            FStation := piece(Results[7], '^', 4); // rpk 8/6/2010; future enhancement
//{$ENDIF}
            ...
            FDTime := Results[11];
            FAgencyCode := Results[27]; // rpk 6/24/2010
            ///
            /// FAgencyCode := 'I';  ///// DEBUG ONLY
            ///
        ...
        begin  // not CreateContext('PSB GUI CONTEXT - USER') then // this is a user
          if assigned(FLogErrorProc) then
