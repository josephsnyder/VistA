...
def parseFieldLine(line,foundFile,foundEntity):
  ...
  if line.split(" ")[0] in allEntities.keys():
    foundFile = line.split(" ")[0]
      ...
      if match.groups()[0]+"_file"   not in allEntities.keys():
...
def parseFile(inFile,out,inCrossRef=None):
  ...
  with open(os.path.join(out,base.replace('.sml','.json')),'w') as outFile:
    outJSON= {"name":base.replace('.sml',''), "children":[]}
    for entry in sorted(allEntities,key=lambda entry: "isNested" in allEntities[entry]):
      foundEntry = False
      if outJSON["name"] in entry or ("isNested" in allEntities[entry]):
        outJSONEntry = outJSON
        if "isNested" in allEntities[entry]:
          for child in outJSON["children"]:
              if child['name'] == entry:
                outJSONEntry = child
                foundEntry = True
                break
          else:
            outJSONEntry = {"name":entry, "children":[]}
        for connectionValue in allEntities[entry]["subfiles"]:
            outJSONEntry['children'].append({"name":connectionValue, "children":[]})
        for connectionValue in allEntities[entry]["fields"]:
            outJSONEntry['children'].append({"name":connectionValue['name'], "children":[]})
        if "isNested" in allEntities[entry] and not foundEntry:
          outJSON["children"].append(outJSONEntry)
    outFile.write(json.dumps(outJSON))

if __name__ == '__main__':
  parseFile("D:/wamp/www/Product-Management/Visual/files/files_json/HOSPITAL_LOCATION.sml",'D:/wamp/www/Product-Management/Visual/files/files_json')