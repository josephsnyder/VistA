...
def run(args):
    ...
    filename = os.path.basename(args.xlsfile)[:-4] # Remove '.txt'
    curDate = filename[filename.rfind("_")+1:]
    RequirementsXLStoJSON.convertExcelToJson(args.xlsfile,args.ReqJsonFile,args.pastData,curDate)
...
if __name__ == '__main__':
    ...
    parser.add_argument('xlsfile', help='path to the VistA Requirements XLS file')
    parser.add_argument('outdir', help='path to the output web page directory')
    parser.add_argument('-p','--pastData',help="The Previous version of the requirements JSON", required=False, default=None)
