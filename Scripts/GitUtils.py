...
def addChangeSet(gitRepoDir=None, patternList=[]):
  ...
  for file in totalIncludeList:
    git_command = git_command_list + [codecs.escape_encode(file)]
...
def _runGitCommand(gitCmdList, workingDir):
  ...
  try:
    ...
    if popen.returncode != 0: # command error
      return (False, codecs.decode(output, encoding='utf-8', errors='ignore'))
    return (True, codecs.decode(output, encoding='utf-8', errors='ignore'))
